...
float printdata[width][height][3];
int nthreads;
scene_t scene;
...
void *thread_func(void *thread_id) {
    ...
    for(int px = *(int*)thread_id; px<width; px += nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            printdata[px][py][0] = scaled_color[0];
            printdata[px][py][1] = scaled_color[1];
            printdata[px][py][2] = scaled_color[2];
...
main( int argc, char **argv )
{
    ...
    for(int i = 0; i < width; i++) {
        for(int j = 0; j < height; j++) {
            printf("%.0f %.0f %.0f\n",
                printdata[i][j][0], printdata[i][j][1], printdata[i][j][2]);
        }
        printf("\n");
    }
